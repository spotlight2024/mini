...
class SeleniumWebDriver(BaseDriver):
    ...
    def connect(self, serial_id: str) -> bool:
        try:
            ...
            self.driver = connect_webdriver(serial_id)
            logging.info(f"WebDriver connected: current url = {self.driver.current_url}")
            return True
        except WebDriverException as e:
            logging.error(f"WebDriver connect error: {e}")
            return False
    ...
    def find_element(self, serial_id: str, selector: str) -> dict:
        if not self.driver:
            return {"code": "fail", "message": "No session found"}
        try:
            element = self.driver.find_element(selector, selector)
            self.driver.implicitly_wait(3)
            return {
                "code": "success",
                "element": {"id": element.id, "tag": element.tag_name, "text": element.text},
                "message": "Found",
            }
        except Exception as e:
            return {"code": "fail", "message": str(e)}

    def find_elements(self, serial_id: str, selector: str) -> dict[str, str] | dict[str, str | list[WebElement]]:
        if not self.driver:
            return {"code": "fail", "message": "No session found"}
        try:
            elements = self.driver.find_elements(selector, selector)
            self.driver.implicitly_wait(3)
            return {
                "code": "success",
                "elements": elements,
